...
module.exports.events.message = (bot, message) => {
  ...
  if (args[1].toLowerCase() === 'nudes')
    return message.author.send('Fuck you.').then(m => message.channel.send('Check your DMs~'))

  if (args[0] === 'send-disable' && message.member && message.member.roles.has('244328249801310219'))
    return message.channel.send(bot.sleet.unloadModule('send'))

  if (args.length < 2)
    return message.channel.send('a')

  let channel = (args[1].match(/<?#?\d+>?/) === null) ? message.channel.id : args[1].match(/<?#?(\d+)>?/)[1]
  ...
  if (!channel.permissionsFor(message.author).has('SEND_MESSAGES'))
    return message.channel.send("I'm not letting you use me to send messages in places where you can't.")
