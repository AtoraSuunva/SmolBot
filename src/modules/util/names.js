...
const promiseTimeout = (time, result = null) => new Promise(r => setTimeout(r, time, result))
...
module.exports.events.message = (bot, message) => {
  ...
  Promise.race([promiseTimeout(10 * 1000), message.guild.fetchMembers()]).then(members => {
    if (members === null)
      return message.channel.send('I did not get the members in less than 10 seconds. Blame Discord and try again later.')

    if (['names', 'n'].includes(cmd))
      count = names(word.toLowerCase(), message.guild.members)
    else
      count = namesStart(word.toLowerCase(), message.guild.members)
...
function names(word, members) {
  ...
  members.array().forEach(m => {
    if (m.user.username.toLowerCase().includes(word) ||
...
function namesStart(word, members) {
  ...
  members.array().forEach(m => {
    if (m.user.username.toLowerCase().startsWith(word) ||
