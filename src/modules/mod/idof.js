module.exports.config = {
  ...
  help:
    'Fetches the id of a mentionned user/by username/tag. Useful for mobile.',
  expanddedHelp:
...
module.exports.events.message = async (bot, message) => {
  ...
  message.guild.members.cache.forEach(m => {
    ...
    if (
      m.user.username.toLowerCase() === user ||
      m.displayName.toLowerCase() === user
    )
  ...
  if (cmd !== 'idof?' && exactResults.length === 0)
    prompt = `No exact matches found, is one of these close?: `
