...
module.exports.events.message = async (bot, message) => {
  ...
  if (Number.isNaN(days) || days < 0 || days > 7)
    return message.channel.send('That is not a valid number of days to delete messages for (1-7)')
  ...
  if (previouslyBanned) {
    try {
      ...
      return message.channel.send(`There was an error while trying to unban that user to reban them.\n\`${e}\``)
    }
  }
  ...
  message.guild.ban(id, {reason})
    ...
    .then(async u => {
        ...
        message.channel.send(`I have softbanned ${bot.sleet.formatUser(user)}, clearing ${days} day${days === 1 ? '' : 's'}.`)
    })
    .catch(e => message.channel.send(`There was an error while trying to softban that user.\n\`${e}\``))
}
