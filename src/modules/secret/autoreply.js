...
const responses = new Map()
  .set('aaa', {
    type: 'is',
    reply: () => 'a'.repeat(Math.floor(Math.random() * 20) + 1),
  })
...
module.exports.events.everyMessage = (bot, message) => {
  ...
  for (const [key, val] of responses) {
    switch (val.type) {
      case 'is':
        if (message.content === key) return sendReply(val, message)
        break

      case 'starts':
        if (message.content.startsWith(key)) return sendReply(val, message)
        break

      case 'ends':
        if (message.content.endsWith(key)) return sendReply(val, message)
        break

      case 'regex':
        if (key.test(message.content)) return sendReply(val, message)
        break
...
function sendReply(val, message) {
  ...
  return message.channel.send(
    typeof val.reply === 'function' ? val.reply(message) : val.reply,
  )
}
