import { ApplicationCommandOptionType, LimitedCollection } from 'discord.js'
...
export const automodBackoff = new AutomodRule(
  {
  ...
  },
  {
    ...
    async autoModerationActionExecution(execution): Promise<AutomodEventResult> {
      const { params } = getAutomodStore<typeof automodBackoff>()
      ...
      if (params.trigger_after > 0 && triggerCount >= params.trigger_after) {
        const user = await this.client.users.fetch(execution.userId).catch(() => null)

        if (!user) {
          return
        }

        const rule = await execution.guild.autoModerationRules
